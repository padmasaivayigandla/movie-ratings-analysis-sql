...
-- Solution 2
SELECT COUNT(CASE WHEN title IS NULL THEN id END) AS title_nulls,
...
SELECT EXTRACT( month from date_published) as month_num, COUNT(*) AS number_of_movies FROM movie GROUP BY month_num ORDER BY month_num;
...
SELECT COUNT(*) AS number_of_movies FROM movie WHERE year=2019 AND (country LIKE '%USA%' OR country LIKE '%India%');
...
WITH summary AS
(SELECT genre, COUNT(movie_id) AS movie_count, RANK () OVER (ORDER BY COUNT(movie_id) DESC) AS 
	genre_rank FROM genre GROUP BY genre)
SELECT genre FROM summary WHERE genre_rank = 1;
...
(SELECT movie_id, COUNT(genre) AS genre_count FROM genre GROUP BY movie_id)
SELECT COUNT(DISTINCT movie_id) AS single_genre_movie_count FROM movie_genre_summary WHERE genre_count=1;
...
WITH top_movies AS
(SELECT m.title, avg_rating, RANK() OVER (ORDER BY avg_rating DESC) AS movie_rank
...
FROM movie AS m INNER JOIN genre AS g ON m.id =g.movie_id 
	INNER JOIN ratings AS r ON m.id = r.movie_id
WHERE title like 'The%' AND avg_rating>8 ORDER BY genre, avg_rating DESC;
...
GROUP BY m.production_company)
SELECT * FROM top_prod WHERE prod_company_rank <= 3;
...
WHERE LOWER(genre) = 'thriller' AND total_votes > 25000 ORDER BY r.avg_rating desc;
...
GROUP BY m.production_company)
SELECT * FROM top_prod WHERE prod_company_rank <= 2;
...
FROM movie_summary
GROUP BY director_id ORDER BY number_of_movies DESC, avg_rating DESC;